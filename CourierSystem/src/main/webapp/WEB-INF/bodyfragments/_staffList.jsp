...
<c:url var="addUrl" value="/home/login/staff" />

<c:url var="addSearch" value="/home/login/staff/search" />

<c:url var="editUrl" value="/home/login/staff?id=" />
	...
	<section class="register">
		...
		<!-- <div class="card shadow">
			<div class="card-body">-->
				...
				<sf:form method="post"
					action="${pageContext.request.contextPath}/home/login/staff/search"
					modelAttribute="form">
					<br />
					...
					<table class="table  rounded table-bordered table-hover table-striped" >
						...
						<tbody>
							<c:forEach items="${list}" var="b" varStatus="staff">
								<tr>
									<c:if test="${sessionScope.user.roleId == 1}">
										<td><input type="checkbox" class="case" name="ids"
											value="${b.id}"></td>
									</c:if>
									<td scope="row">${staff.index+1}</td>
									<td scope="row">${b.firstName}</td>
									<td scope="row">${b.lastName}</td>
									<td scope="row">${b.login}</td>
									<%-- <td scope="row">${b.branches.street}</td> --%>
